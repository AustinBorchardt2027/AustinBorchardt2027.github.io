...
const STORAGE_KEYS = {
  SHEET_URL: 'cinematheque_sheet_url',
  SCRIPT_URL: 'cinematheque_script_url',
};
...
async function fetchWithCORS(url) {
  ...
  // Fallback: try with no-cors (won't help for JSON but good for CSV)
...
async function loadData(sheetURL, scriptURL) {
  ...
  // ── 1. Fetch CSV ──
  try {
    const r = await fetchWithCORS(sheetURL);
  ...
  // ── 2. Fetch Drive JSON ──
  try {
    const r = await fetchWithCORS(scriptURL);
    driveMap = await r.json();
    setProgress(80);
  } catch (e) {
...
$('cfg-save').addEventListener('click', () => {
  const sheet  = $('cfg-sheet').value.trim();
  const script = $('cfg-script').value.trim();

  if (!sheet || !script) {
    showToast('Please fill in both URLs.');
    return;
  }

  localStorage.setItem(STORAGE_KEYS.SHEET_URL, sheet);
  localStorage.setItem(STORAGE_KEYS.SCRIPT_URL, script);
  configModal.classList.add('hidden');
  loadData(sheet, script);
});
...
function loadDemoData() {
  ...
  setTimeout(() => {
    ...
    setProgress(100);
    setTimeout(() => scanBar.classList.add('hidden'), 500);
    render();
    populateResFilter();
    updateCounts();
...
(function init() {
  const savedSheet  = localStorage.getItem(STORAGE_KEYS.SHEET_URL);
  const savedScript = localStorage.getItem(STORAGE_KEYS.SCRIPT_URL);

  if (savedSheet && savedScript) {
    // Pre-fill and auto-load
    $('cfg-sheet').value  = savedSheet;
    $('cfg-script').value = savedScript;
    configModal.classList.add('hidden');
    loadData(savedSheet, savedScript);
  } else {
    // Show config modal
